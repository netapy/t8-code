  ...
  switch (command.type) {
    ...
    case "thread.compact": {
      ...
      if (thread.session?.status === "running") {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Thread '${command.threadId}' is running and cannot compact until the turn finishes.`,
        });
      }
      return {
        ...withEventBase({
    ...
    case "thread.message.import": {
      ...
      return {
        ...withEventBase({
          aggregateKind: "thread",
          aggregateId: command.threadId,
          occurredAt: command.message.updatedAt,
          commandId: command.commandId,
        }),
        type: "thread.message-sent",
        payload: {
    ...
    case "thread.follow-up.dispatch-next": {
      ...
      if (
        ...
        followUp.modelSelection !== thread.modelSelection
      ) {
        events.push({
          ...withEventBase({
            aggregateKind: "thread",
