...
export default function ChatView(props: ChatViewProps) {
  ...
  const onSteer = useCallback(
    async (text: string): Promise<boolean> => {
      ...
      try {
        ...
        setPrompt(trimmed);
        composerRef.current?.resetCursorState({
          cursor: collapseExpandedComposerCursor(trimmed, trimmed.length),
          prompt: trimmed,
          detectTrigger: true,
        });
        setThreadError(
  ...
  const onEditQueuedFollowUp = useCallback(
    ...
    [activeThread, composerRef, onRemoveQueuedFollowUp, scheduleComposerFocus, setPrompt],
  );
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className={cn("px-3 pt-1.5 sm:px-5 sm:pt-2", isGitRepo ? "pb-1" : "pb-3 sm:pb-4")}>
            ...
            pendingUserInputs.length === 0 ? (
              <div className="mx-auto mt-2 w-full min-w-0 max-w-[52rem]">
                    ...
                    {activeQueuedFollowUps.map((followUp, index) => (
                      <div key={followUp.messageId} className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            void onEditQueuedFollowUp(followUp.messageId, followUp.text)
                          }
                        >
                          Edit
                        </Button>
